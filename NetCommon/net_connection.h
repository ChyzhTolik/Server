	...
	namespace net
	{
		...
		public:
				...
				: m_asioContext(asioContext), m_socket(std::move(socket)), m_qMessagesIn(qIn)
			{
				...
				{
					//Connection is Client -> Server, so we have nothing o define
					m_nHandshakeIn = 0;
					m_nHandshakeOut = 0;
				}
			}
			...
			void ConnectToClient(olc::net::server_interface<T>* server, uint32_t uid = 0)
			{
				if (m_nOwnerType == owner::server)
				{
					if (m_socket.is_open())
					{
						...
						// A client has attrmpted to connect to the server, but we wish
					...
					asio::async_connect(m_socket, endpoints,
						[this](std::error_code ec, asio::ip::tcp::endpoint endpoint)
						{
							if (!ec)
							{
								...
								// First thing server will do is srnd packrt to be validated
